...
extension BrowserState {
    ...
    func consumePendingSplitPartner(for tab: Tab) {
        ...
        // Inherit the partner's tab group on the new pane. The
        // existing-tab → grouped-tab drag path gets this for free because
        // both tabs were already grouped (or not) before `createSplit`; the
        // deferred-new-tab paths (bookmark drop, closed-pinned drop, sidebar
        // "open as split") materialize a fresh ungrouped tab and need an
        // explicit join. Chromium's `AddTabsToGroup` expand-to-cover-splits
        // logic adding the new tab is enough — the partner is already a
        // member, so passing it would be a no-op for that pane.
        if createdSplitId != nil,
           let partner = tabs.first(where: { $0.guid == pending.partnerTabId }),
           let partnerToken = partner.groupToken {
            ChromiumLauncher.sharedInstance().bridge?
                .addTabsToGroup(withWindowId: windowId.int64Value,
                                tabIds: [NSNumber(value: Int64(tab.guid))],
                                tokenHex: partnerToken)
            // Same MainActor situation as the `focuseTab` call above:
            // `applyOptimisticGroupMembership` is statically @MainActor but
            // this whole flow already runs on main via EventBus dispatch.
            MainActor.assumeIsolated {
