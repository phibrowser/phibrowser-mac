...
- 2026-09-20 (one sheet per toggle, nothing else): the pane is the three toggles with their summary lines; no footer buttons. Each toggle's `ContentBlockingRuleSetSheet` lists the toggle's catalog lists and, below, the profile's custom lists (shared by all three toggles) with "Add Filter…"; "Update Downloaded" sits at the sheet's bottom left and refetches the lists shown there that are on disk (catalog and custom URL lists), Done at the bottom right. The separate Custom Filters sheet is gone; `ContentBlockingListRow` now lives in the rule set sheet file; `ContentBlockingListRowTests` holds the row tests. The bridge's `refreshContentBlockingLists:` stays but the UI no longer calls it.

- 2026-09-20 (Advanced sheet replaced by Custom Filters): each thing now has one place. Catalog lists are chosen and downloaded in the per-toggle `ContentBlockingRuleSetSheet` only; custom lists live in `ContentBlockingCustomFiltersSheet` (opened by "Custom Filters…" in the pane footer, next to "Update Downloaded"); `ContentBlockingSection` / the grouped Advanced sheet and their strings are gone. The summary under a toggle reads the list's name for one list, "N rule sets" for more, plus "1 custom list" / "N custom lists". Tests: `ContentBlockingCustomFiltersSheetTests` replaces `ContentBlockingAdvancedSheetTests`.

- 2026-09-20 (Advanced sheet aligned with the chooser): no section is disabled by its toggle (lists can be chosen before the toggle is on); Done downloads every checked list still missing, across sections, and closes once they land (`ContentBlockingAdvancedSheet.missingIds`); the delete-download control is gone because the files are shared by every profile and deleting one would switch other profiles' toggles off (the bridge method stays, unused); "Update All" is "Update Downloaded" and `RefreshLists` now re-downloads every list on disk (catalog plus this profile's custom URL lists), not just the enabled ones; toggles turn on only with a usable rule set and turn off when the last one goes (`ContentBlockingRuleSets.togglesToTurnOff`).

- 2026-09-20 (choice vs download settled with the owner): the checkbox means "use", the ↓ means "download", the sheet's only button is Done (choices apply immediately). A catalog default (or language default) counts as chosen only once the list is on disk, so a fresh profile shows nothing chosen; an explicit choice shows regardless. Deleting a download removes the user's override so the default rule applies again on the next download. The bridge's `checked` implements this (`SnapshotOf`); `DeleteListDownload` removes the override instead of writing false. The rows tag recommended lists until downloaded.

- 2026-09-20 (catalog trimmed to publisher hosts): only easylist.to, secure.fanboy.co.nz and filters.adtidy.org remain as sources; the regional set follows the Mac client's localizations (de, fr, nl, es+pt, ja, zh; AdGuard has no Korean filter). Dropped: uBlock Ads / Unbreak / Privacy (GitHub raw only), Polish (GitHub, CC BY-NC-SA), Bulgarian (personal site, no license). Consequence to watch: uBlock Unbreak was the breakage-fix list; sites broken by EasyList now rely on the site exception or on a custom list. `engine_perftest` reads lists from `--phi-content-blocking-lists-dir`.

- 2026-09-20 (explicit downloads only, owner decision): nothing is fetched on its own. `ListStore` lost its daily refresh and hourly timer; `Download(ids)` fetches missing files and `Refresh(ids)` fetches again, both only when called. The service never asks for downloads in `Reconcile`; new status `kNoLists` ("no_lists" on the bridge) means a toggle is on with no downloaded list, with the last download error in the detail. Explicit paths: `ContentBlockingService::DownloadLists` (bridge `downloadContentBlockingLists:listIds:`), `RefreshLists` (Update All), adding a custom URL list, and the per-row download button. Mac: turning a toggle on whose lists are not on disk opens `ContentBlockingRuleSetSheet` (checkboxes, per-row download state, Cancel reverts the toggle, Download fetches the checked missing ones); each toggle shows a summary line of its checked lists and their state (`ContentBlockingRuleSets.summary`) that opens the same sheet; checking a list in the Advanced sheet no longer downloads it. Tests: `ContentBlockingRuleSetSheetTests`, updated `list_store_unittest.cc` and `content_blocking_service_unittest.cc`.
- 2026-09-20 (per-list download controls): each Advanced sheet row ends in one control chosen by `ContentBlockingListRow.control(for:)`: download (missing or failed), a spinner (fetching), delete download (on disk; Chromium also unchecks the list so it is not fetched straight back), or remove (custom lists); an "Update All" button in the sheet header calls `refreshLists`. Chromium: `ListStore::DeleteDownload`, `ContentBlockingService::{DownloadList, DeleteListDownload}`, bridge `downloadContentBlockingList:` / `deleteContentBlockingListDownload:`. The intro copy no longer mentions the removed Privacy pane.
...
- Catalog list ids (since 2026-09-20, publisher hosts only): `easylist`, `easyprivacy`, `easylist-cookie`, `adguard-german`, `adguard-french`, `adguard-dutch`, `adguard-spanish`, `adguard-japanese`, `adguard-chinese`, `phi-specific`; the uBlock, Polish and Bulgarian lists were dropped (GitHub raw or unlicensed), users add them as custom lists. Categories: `ads`, `trackers`, `cookies`, `regional`, `custom`, `phi`.
