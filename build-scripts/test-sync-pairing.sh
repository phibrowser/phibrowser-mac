...
manager = (root / 'Sources/Sync/Keys/AccountKeyManager.swift').read_text()
a = manager.index('    func discardLocalRegistration()')
b = manager.index('\n    }', a) + len('\n    }')
fixture = fixture.replace('    /* PRODUCTION_DISCARD_REGISTRATION */', manager[a:b])
marker = (root / 'Sources/Sync/Phi/PhiSyncMarkerFile.swift').read_text()
a = marker.index('struct PhiSyncMarkerFile:')
b = marker.index('/// Class-bound', a)
